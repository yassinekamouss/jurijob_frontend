...
// --- Composant Avatar natif ---
// Pour gérer l'image et le fallback, nous créons un petit composant helper.
...
function NativeAvatar({ src, fallbackText, className, fallbackClassName }: NativeAvatarProps) {
  ...
  return (
    <div className={`relative inline-flex shrink-0 overflow-hidden rounded-full ${className}`}>
      {!showFallback ? (
        ...
        <div className={`flex h-full w-full items-center justify-center rounded-full ${fallbackClassName}`}>
          {fallbackText}
        </div>
      )}
    </div>
...
// --- Composant principal ---
export function PersonalInfoSection({ data, onSave }: PersonalInfoSectionProps) {
  ...
  const editedInitials = `${editedData.prenom?.[0] || "?"}${editedData.nom?.[0] || "?"}`.toUpperCase();
            ...
            <div className="flex-1 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                <div className="space-y-1.5">
                  ...
                  <p className="text-black pl-5">{data.prenom} {data.nom}</p>
                </div>
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-gray-500 text-sm">
              ...
              <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
                <Power className={`h-4 w-4 ${data.isActive ? "text-green-600" : "text-gray-400"}`} />
                <span className="text-sm text-gray-600">Profil</span>
                <span className={`text-sm ${data.isActive ? "text-green-600" : "text-gray-500"}`}>
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="flex-1 space-y-4">
                <div className="space-y-2">
                  <label htmlFor="imageUrl" className="text-sm font-medium text-gray-700">Photo de profil</label>
                  <input
                    id="imageUrl"
            ...
            <div className="pt-4 border-t border-gray-200">
              <div className="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  <Power className={`h-5 w-5 ${editedData.isActive ? "text-green-600" : "text-gray-400"}`} />
                  <div>
                    <p className="text-sm text-gray-900">Statut du profil</p>
                    <p className="text-xs text-gray-500">
                      {editedData.isActive ? "Votre profil est visible par les recruteurs" : "Votre profil est masqué"}
                    </p>
                  </div>
                </div>
                ...
                <button
                  ...
                  onClick={() => setEditedData({ ...editedData, isActive: !editedData.isActive })}
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 ${editedData.isActive ? 'bg-black' : 'bg-gray-200'}`}
                >
                  <span
                    aria-hidden="true"
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${editedData.isActive ? 'translate-x-5' : 'translate-x-0'}`}
                  />
                </button>
              </div>
            </div>
